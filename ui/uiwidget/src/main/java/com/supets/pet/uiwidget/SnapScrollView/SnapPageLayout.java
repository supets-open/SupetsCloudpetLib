...
/**
 * Created by WeiDongliang on 2016/1/8.
 */
public class SnapPageLayout extends ViewGroup {
    ...
    private SnapPageContent mPageTop, mPageBottom;
    ...
    public interface SnapPageContent {
        /**
         * 返回page根节点
         *
         * @return
    ...
    public SnapPageLayout(Context context, AttributeSet att) {
        this(context, att, 0);
    }

    public SnapPageLayout(Context context, AttributeSet attrs, int defStyle) {
...
//		 DLog.i(TAG, "onLayout mDataIndex = " + mDataIndex);
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != View.GONE) {
                ...
                child.layout(0, childTop, childWidth,
                        childTop + childHeight); //这里调用了第二页的scroll
            }
        }
        if(count > 0){
     ...
     * @param pageBottom
     */
    public void setSnapPages(SnapPageContent pageTop, SnapPageContent pageBottom) {
        mPageTop = pageTop;
        mPageBottom = pageBottom;
        addPagesAndRefresh();
    }

    public void setSnapTopPage (SnapPageContent pageTop) {
        mPageTop = pageTop;
        addPageAndRefresh2();
    }

    public void setSnapBottomPage (SnapPageContent pageTop) {
        mPageBottom = pageTop;
        addPageAndRefresh3();
    }
    ...
    @SuppressWarnings("ResourceType")
    private void addPageAndRefresh2() {
        mPageTop.getRootView().setId(0);
        addView(mPageTop.getRootView());
        postInvalidate();
    }
    @SuppressWarnings("ResourceType")
    private void addPageAndRefresh3() {
        mPageBottom.getRootView().setId(1);
        addView(mPageBottom.getRootView());
        postInvalidate();
    }
    ...
    private void snapToDestination() {
        // 计算应该去哪个屏
        final int flipHeight = getHeight() / 4;
        ...
        Log.e(TAG, "snapToDestination mDataIndex = " + mDataIndex);//1
        Log.e(TAG, "snapToDestination whichScreen = " + whichScreen);//0
    ...
    private void snapToScreen(int dataIndex) {
        ...
        if (focusedChild != null && changingScreens) {
            focusedChild.clearFocus(); //这里调用了第二页的scroll
        }
        //在这里判断是否已到目标位置~
        int newY = 0;
        switch (direction) {
